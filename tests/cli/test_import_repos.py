...
def test_import_prune_same_source_across_workspaces(
    ...
    Case 1: Remote returns ``myrepo`` and the import targets ``~/code/`` →
    that workspace keeps ``myrepo``, while the same-named stale copy in the
    non-target ``~/projects/`` is pruned. Prune is scoped to the import
    target, so an entry not imported into this run is stale even if a
    same-named repo is still on the remote (see
    ``test_import_prune_cross_workspace_same_source_same_name``).
    ...
    # ── Case 1: remote has myrepo; target keeps it, non-target pruned ──
    ...
    # Target workspace keeps myrepo; the non-target stale copy is pruned.
    assert "myrepo" in final_case1["~/code/"]
    assert "myrepo" not in final_case1.get("~/projects/", {})
